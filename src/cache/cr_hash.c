...
unsigned cr_hash(const char *str)
{
    if (str == NULL || str[0] == '\0') {
        log_error_time();
        fprintf(stderr,
                "Attempt to hash NULL or empty string! [cr_hash]!\n");
        return 0;
    }
    return _cr_hash(str);
}
...
static unsigned get_homedir_hash_value(const char *name)
{
    return cr_hash(name) % PASSWD_HASHTABLE_SIZE;
}
